...
files = dir('/home/Neidhardt/Dokumente/Paper_Neidhardt_Gerlach/Annotated_CTs_II/*/Bauch*/*');
...
for p=1:9
    names_GT{p} = AnnotationFiles{9}(p).name;
    namePath = [AnnotationFiles{9}(p).folder '/' AnnotationFiles{9}(p).name];
    fid = fopen(namePath);
    raw = fread(fid,inf);
    str = char(raw');
    fclose(fid);
...
for vol_shift = 1:size(t_CT,1)
    ...
    if contains(str_V,'Bauch_I_')
        str_V = [str_V(1:pos(end)) 'Bauch_I_XYZ.txt']
    elseif contains(str_V,'Bauch_II_')
        str_V = [str_V(1:pos(end)) 'Bauch_II_XYZ.txt'];
    else
        str_V = [str_V(1:pos(end)) 'Bauch_XYZ.txt'];
    end
...
for vol_shift = 1:size(t_CT,1)
    ...
    str_V_ref = [FoldersVolume{9}.folder '/'];
    str_V_ref = strrep(str_V_ref,'Annotated_CTs_II','Colormaps_160');
    file_ref = dir([str_V_ref 'T_26*']);
	if isempty(file_ref)
